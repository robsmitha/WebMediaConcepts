...
class Itemstatustype {
	...
	public function load($paramId) {
		...
		$stmt = $conn->prepare('CALL usp_itemstatustype_Load(?)');
	...
	private function insert() {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_itemstatustype_Add(?,?)');
		$arg1 = $this->getName();
	...
	private function update() {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_itemstatustype_Update(?,?,?)');
		$arg1 = $this->getId();
		$arg2 = $this->getName();
	...
	public static function loadall() {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_itemstatustype_LoadAll()');
		$stmt->execute();
		...
		if ($result->num_rows > 0) {
			$arr = array();
			while ($row = $result->fetch_assoc()) {
				$statustype = new Itemstatustype($row['Id'],$row['Name'],$row['Description']);
				$arr[] = $statustype;
			}
			return $arr;
		}
	...
	public static function remove($paramId) {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_itemstatustype_Delete(?)');
		$stmt->bind_param('i', $paramId);
		$stmt->execute();
	}
	...
	public static function search($paramId,$paramName,$paramDescription) {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_itemstatustype_Search(?,?,?)');
		$arg1 = Itemstatustype::setNullValue($paramId);
		$arg2 = Itemstatustype::setNullValue($paramName);
		$arg3 = Itemstatustype::setNullValue($paramDescription);
		$stmt->bind_param('iss',$arg1,$arg2,$arg3);
		$stmt->execute();
